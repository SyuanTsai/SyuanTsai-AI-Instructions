Describe 'Agent Skill authority workflow contract' {
    BeforeAll {
        ...
        $script:WorkflowExpectations = [ordered]@{
            '.github/workflows/pr8-powershell-validation.yml' = 5
...
'@
    ...
    It 'UnitT10_pins_every_checkout_and_disables_persisted_credentials' {
        ...
        Assert-Equal ([regex]::Matches($required, '& ./scripts/Invoke-PesterShardProcess\.ps1 @executorArguments')).Count 3 'Both Windows PowerShell 5.1 partitions and the PowerShell 7 suite must use the bounded executor.'
        Assert-Equal ([regex]::Matches($required, 'Executing Pester \$\(\$pester\.Version\) through the bounded shard executor\.')).Count 3 'Workflow logging must use discovery metadata without importing the module first.'
    }
    ...
    It 'UnitT15_runs_linux_callback_containment_before_full_suites_and_authority_gate' {
        ...
        Assert-Equal ([regex]::Matches($required, 'ExpectedTotalCount\s*=\s*619').Count) 1 'PowerShell 7 must still run the complete 619-test suite.'
