...
'@
    ...
    # Scenario: A production adapter carries a freshly signed resolver receipt whose run ID is not available as a caller input.
    # Purpose: Derive the trusted production validation run ID from the authenticated receipt before the full adapter is resolved, then bind every slot to it.
    It 'UnitT01_derives_production_run_id_from_signed_resolver_receipt' {
        ...
        try {
            ...
            Assert-Equal $derivedRunId.ToString('N') $runIdText 'Production run ID must come from the signed resolver receipt.'
