...
7. 每個 AI 都必須具備設定預留額度的能力；同一 AI 的不同帳號、quota profile 或額度來源也必須各自獨立。
8. 使用者必須能依需求持續調整各項預留額度，不能只在初次設定時決定後永久固定；個人 Copilot Review 是目前已知的保障用途之一。
9. 預留額度如何影響 AI 選擇尚未決定；其表示方式、調整介面、約束強度與生效方式也尚未決定。
...
確認後，再依序討論 `unknown`／非 Git 任務與多 Repository 混合任務；如何分配或使用 AI 留待目標邊界完整後再討論。
