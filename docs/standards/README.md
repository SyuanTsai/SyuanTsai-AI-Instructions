...
Production adapter command specs MUST resolve to non-reparse, directly executable trusted files；known generic interpreters and script payloads are development-harness-only and MUST be rejected for production. The runner MUST revalidate the snapshot inventory before and after every child process. `canonicalValidatorPath` MUST identify the one candidate entry point checked by the central consumer entry-point inventory and invoked as the canonical gate. Human approval MUST be a candidate-bound, trusted-supervisor-signed RSA attestation；self-asserted approver/status/timestamp fields are insufficient。
