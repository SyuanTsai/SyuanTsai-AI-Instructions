...
For an adopted surface, the executable adapter MUST receive the immutable `SourceRepository`, `SourceRevision` and `ArchiveSha256` inputs. A `PASS` report emits `adapterVersion`, `candidateIdentity` (`sourceRepository`, `sourceRevision`, `archiveSha256`, and the computed `packageSha256`), and an ordinal path-ordered `componentInventory` containing the SHA-256 of every validated manifest, package-local MCP command and declared Skill resource. `componentInventorySha256` is the SHA-256 of the canonical `<path>\t<sha256>\n` inventory. The adapter re-hashes the component paths after validation and blocks if any snapshot changed, so a report cannot be replayed against a different set of package bytes. App mappings MUST reference an MCP identity declared by the same package; an arbitrary host or undeclared server name is not valid evidence.
