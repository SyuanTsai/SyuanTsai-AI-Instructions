...
The central runner **MUST** generate the fresh validation run ID；a caller-supplied `-RunId` **MUST** be rejected outside the development harness so an old `resolutionRunId` cannot be replayed into a new validation。
