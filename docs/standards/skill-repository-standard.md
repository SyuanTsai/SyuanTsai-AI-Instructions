...
The trusted resolution orchestration **MUST** generate a fresh validation run ID and sign it in the resolver receipt；the central runner **MUST** authenticate the signed `package-adapter` receipt first, derive the production run ID from its `resolutionRunId`, and require every production adapter slot to bind to that same ID. A caller-supplied `-RunId` **MUST** be rejected outside the development harness so an old or caller-selected `resolutionRunId` cannot be replayed into a new validation. The runner **MUST** capture each child stdout and stderr stream with a fixed maximum of 1,048,576 characters；when either stream exceeds its quota, it **MUST** terminate the complete owned process tree, retain only the bounded prefix, and fail the event；it **MUST NOT** use unbounded `ReadToEnd` capture。
