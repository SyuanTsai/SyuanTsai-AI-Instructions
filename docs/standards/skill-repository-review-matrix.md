...
The Requires-Python policy parses zero or one wheel `METADATA` field for the root and every dependency, rejects invalid or current-interpreter-incompatible specifiers, and requires an approved Simple JSON `requires-python` value to match the wheel field exactly after trimming outer whitespace. A missing/null index value is accepted only when the wheel also omits the field, so pip never sees a candidate whose index compatibility claim is missing or inconsistent with its embedded metadata.
