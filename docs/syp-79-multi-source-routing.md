...
The legacy direct entry point remains only for manifest-v1 regression compatibility. The installer deploys `bootstrap-ai-instructions-installed.ps1`, which always routes through the multi-source wrapper. Manifest v1 upgrades only when every legacy managed file is unchanged.
...
SYP-86 adds installer-driven schema v3 migration, the final installed multi-source entry point, tracked production lock with a CI stale gate, safe ZIP extraction, manifest v2 provenance, and v1 migration protection. Removal of legacy Skill copies remains a final optional cleanup only after all acceptance gates in `docs/syp-86-production-cutover.md` pass.
