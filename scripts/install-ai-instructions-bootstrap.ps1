...
# Stored as ASCII Base64 so Windows PowerShell 5.1 cannot reinterpret the UTF-8 text
# through the active ANSI code page while parsing this installer.
...
function Get-SyncConfigurationJson {
    ...
    if ($existingSchemaVersion -eq 3) {
        ...
        if ($null -eq $existingIdentity -or $existingIdentity -cne $installedIdentity) {
            throw 'AI instruction sync configuration catalog.repository must identify the same GitHub AI-Instructions repository as the installed runtime bundle.'
        }
...
function Assert-InstallerSourcesMatchHead {
    ...
    foreach ($relativePath in $RelativePaths) {
        Invoke-Git -WorkingDirectory $Repository -Arguments @('ls-files','--error-unmatch','--',$relativePath) | Out-Null
    }
    ...
    try {
        $ErrorActionPreference = 'Continue'
        $changed = & git -C $Repository @arguments 2>&1
        $exitCode = $LASTEXITCODE
    }
    finally {
        $ErrorActionPreference = $previousErrorActionPreference
    ...
    if ($exitCode -ne 0) {
        throw "Unable to verify installer source files against HEAD: $($changed -join [Environment]::NewLine)"
    }
    if (@($changed).Count -gt 0) {
        throw "Installer source files differ from HEAD and cannot be represented by the immutable install pin: $(@($changed) -join ', ')"
    }
}
...
$relativeSourcePaths += 'catalog/skills-catalog.json'
$relativeSourcePaths += 'catalog/skills-catalog-lock.json'
...
foreach ($fileName in $runtimeFiles) {
    Copy-Item -LiteralPath (Join-Path $repositoryRootPath "scripts\$fileName") -Destination (Join-Path $stagingRuntime $fileName) -Force
}
...
$bundleJson = ([ordered]@{
    ...
    repository = $catalogRepository
    commit = $catalogRef
} | ConvertTo-Json -Depth 4).Replace("`r`n","`n") + "`n"
Write-Utf8NoBomFile -Path (Join-Path $stagingRuntime 'runtime-bundle.json') -Content $bundleJson

$configurationJson = Get-SyncConfigurationJson -ConfigurationPath $configurationPath `
...
Write-Utf8NoBomFile -Path $stagingConfiguration -Content $configurationJson

# Validate the complete staged bundle before touching the active runtime.
$stagedConfig = Get-Content -Raw -Encoding UTF8 -LiteralPath $stagingConfiguration | ConvertFrom-Json
$stagedBundle = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $stagingRuntime 'runtime-bundle.json') | ConvertFrom-Json
if ([string]$stagedConfig.catalog.repository -cne [string]$stagedBundle.repository -or
    [string]$stagedConfig.catalog.ref -cne [string]$stagedBundle.commit) {
    throw 'Staged runtime bundle identity does not match staged configuration.'
}
Import-Module (Join-Path $stagingRuntime 'skills-catalog-contract.psm1') -Force
Test-SkillsCatalogLockDocument `
    -LockPath (Join-Path $stagingRuntime 'catalog\skills-catalog-lock.json') `
    -CatalogPath (Join-Path $stagingRuntime 'catalog\skills-catalog.json') | Out-Null
foreach ($scriptPath in @($stagingLauncher) + @(Get-ChildItem -LiteralPath $stagingRuntime -File -Filter '*.ps*' | Select-Object -ExpandProperty FullName)) {
    $tokens = $null
    $errors = $null
    [void][System.Management.Automation.Language.Parser]::ParseFile($scriptPath, [ref]$tokens, [ref]$errors)
    if (@($errors).Count -gt 0) {
        throw "Staged installer runtime contains a PowerShell parse error in '$scriptPath': $(@($errors)[0].Message)"
    }
}

$hadRuntime = Test-Path -LiteralPath $runtimeDirectory -PathType Container
$hadHook = Copy-BackupFileIfPresent -Source $hookScript -Destination (Join-Path $backupRoot 'bootstrap-ai-instructions.ps1')
$hadConfiguration = Copy-BackupFileIfPresent -Source $configurationPath -Destination (Join-Path $backupRoot 'ai-instructions-sync.json')
$hadAgents = Copy-BackupFileIfPresent -Source $agentsPath -Destination (Join-Path $backupRoot 'AGENTS.md')
$hadHooks = Copy-BackupFileIfPresent -Source $hooksPath -Destination (Join-Path $backupRoot 'hooks.json')
$runtimeBackedUp = $false
$runtimeInstalled = $false
$installSucceeded = $false

try {
    # Install the identity-checking launcher first. Until the remaining swap completes it fails closed.
    ...
    Move-Item -LiteralPath $stagingRuntime -Destination $runtimeDirectory
    $runtimeInstalled = $true

    # Configuration is committed last so the active pin cannot advance before the complete runtime exists.
    ...
    $installError = $_
    try {
        if ($runtimeInstalled -and (Test-Path -LiteralPath $runtimeDirectory -PathType Container)) {
            Remove-Item -LiteralPath $runtimeDirectory -Recurse -Force
        }
        if ($runtimeBackedUp -and (Test-Path -LiteralPath $backupRuntime -PathType Container)) {
        ...
        throw "AI instructions installation failed and rollback also failed. Original error: $($installError.Exception.Message). Rollback error: $($_.Exception.Message)"
    }
    throw $installError
}
finally {
