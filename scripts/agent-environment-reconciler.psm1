...
function Restore-AgentEnvironmentFileState {
    ...
    if ([bool]$State.existed) {
        if (Test-Path -LiteralPath $full -PathType Leaf) {
            ...
            if ($actual -ceq [string]$State.originalSha256) { return }
        ...
        Set-AgentEnvironmentFileBytes -Root $Root -RelativePath $relative -Bytes ([System.IO.File]::ReadAllBytes($backupPath))
        return
    }
    if (Test-Path -LiteralPath $full -PathType Leaf) {
        $actual = Get-AgentEnvironmentSha256 -Path $full
...
function Invoke-UserSkillsReconciliation {
    ...
    try {
        ...
        foreach ($path in @($oldByPath.Keys)) {
            ...
            if (-not $desiredByPath.ContainsKey($path) -and (Test-Path -LiteralPath $full -PathType Leaf)) { $deletes[$path] = $true; $removed.Add($path) }
        }
        ...
        foreach ($path in $mutationPaths) {
            ...
            if ($existed) {
                ...
                $originalSha = Get-AgentEnvironmentBytesSha256 -Bytes $originalBytes
            }
            $appliedSha = $null
...
function Invoke-UserSkillsRecovery {
    ...
    try {
        ...
        if ([int]$journal.schemaVersion -ne 1 -or [string]$journal.userHome -cne $home) { throw 'Agent environment recovery journal identity is invalid.' }
        foreach ($state in @($journal.states)) {
            $relative = [string]$state.relativePath
            Restore-AgentEnvironmentFileState -Root $home -State $state
        }
        Remove-Item -LiteralPath $journalPath -Force
