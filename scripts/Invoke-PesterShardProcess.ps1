...
function Get-PesterShardPreflight {
    ...
    $excludedSensitiveNames = @($childEnvironmentNames | Where-Object { $_ -match '(?i)secret|password|token|authorization|credentials?|(?:api|private|access)[-_ ]?key|bearer|cookie|session' })
